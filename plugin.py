...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        any_sensor_active = any(
            self.get_device_nvalue_by_idx(idx) == 1 for idx in self.presence_sensors
        )
    ...
    def get_device_nvalue_by_idx(self, idx):
        try:
            url = f"http://127.0.0.1:8080/json.htm?type=devices&rid={idx}"
            with urllib.request.urlopen(url) as response:
                data = json.loads(response.read().decode())
                return data['result'][0]['nValue']
        except Exception as e:
            Domoticz.Error(f"Erreur en lisant l'idx {idx} : {e}")
            return 0
